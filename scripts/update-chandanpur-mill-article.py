...
post_file = os.path.join(base_dir, 'content', 'posts', 'chandanpur-sugar-factory-2026.md')
...
site_img = os.path.join(base_dir, 'static', 'images', 'blog', 'chandanpur-sugar-factory-2026.webp')
...
# Parse raw villages
raw_file = os.path.join(base_dir, 'scripts', 'chandanpur_raw.txt')
...
for val, text in matches:
    if val in ["-1", "999999", "261", "262", "263"] or "DUMMY" in text or "NONMEMBER" in text or "TEST" in text or "ALLAAAAAAAAAA" in text:
        continue
...
print(f"Parsed {len(villages)} villages for Chandanpur Sugar Mill.")

rows_html = ""
for idx, (vname, vcode) in enumerate(villages, 1):
    rows_html += f'  <tr class="vrow"><td>{idx}</td><td class="vname"><strong>{vname}</strong></td><td class="vcode"><code style="background:#dcfce7;color:#15803d;padding:2px 8px;border-radius:4px;font-weight:700;">{vcode}</code></td><td>चंदनपुर चीनी मिल (Wave Sugar - Code 142)</td></tr>\n'

seo_title = f"चंदनपुर चीनी मिल 2026-27: {len(villages)} गांवों की लिस्ट, कोड व CaneUp पर्ची कैलेंडर | Chandanpur Sugar Mill Amroha Hasanpur"
seo_desc = f"चंदनपुर चीनी मिल (Chandanpur Sugar Mill Code 142 Amroha Hasanpur) 2026-27 के सभी {len(villages)} गांवों की आधिकारिक सूची व Village Code। 1 सेकंड में गांव खोजें, eGanna पर्ची कैलेंडर व डीबीटी भुगतान स्थिति।"

article_content = f"""---
title: "{seo_title}"
date: 2026-08-29T01:25:00+05:30
lastmod: 2026-08-29T01:25:00+05:30
...
## 🔍 चंदनपुर चीनी मिल — सभी {len(villages)} गांव व कोड लाइव खोजें (Live Search Tool)
...
print(f"Successfully generated SEO-optimized Chandanpur Mill Article with all {len(villages)} villages!")
